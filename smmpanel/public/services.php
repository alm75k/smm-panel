...
// Filtrage côté PHP si GET['category'] est passé
        ...
        <tbody>
            <?php foreach($allServices as $s): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($s->rate ?? '') ?> <?= htmlspecialchars($s->currency ?? '') ?></td>
                    <td><?= htmlspecialchars($s->description ?? '') ?></td>
                    <td>
                        <a href="new_order.php?service_id=<?= $s['service_id'] ?>" class="btn btn-primary btn-sm">Commander</a>

                    </td>
                </tr>
            <?php endforeach; ?>
            <?php if (empty($allServices)) : ?>
                <tr><td colspan="6" class="text-center">Aucun service disponible</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
